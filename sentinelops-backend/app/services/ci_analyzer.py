...
"""
from app.services.embedding_service import embed_log, cosine_similarity
...
class CIAnalyzer:
    ...
    def analyze_build_time(self, current_ms: int, historical: list[int]) -> dict:
        ...
        return {
            ...
            "mean_ms": int(mean),
            "stdev_ms": int(stdev),
            "reason": f"Build time is {abs(z_score):.1f} std deviations from mean" if is_anomalous else "Within normal range"
        }
