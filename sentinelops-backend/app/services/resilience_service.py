...
class ResilienceEngine:
    async def calculate_pulse(self, db: AsyncSession) -> Dict[str, Any]:
        ...
        risk_score = max(
            0, 100 - (high_risk_prs * 20)
        )  # Penalty for unaddressed high risk

        aggregate = (
            (stability_score * 0.4) + (recovery_score * 0.3) + (risk_score * 0.3)
        )
        ...
        return {
            ...
            "status": (
                "EXCELLENT"
                if aggregate > 85
                else "STABLE" if aggregate > 65 else "CRITICAL"
            ),
            "metrics": {
